...
2. **Ingest a repository**: *Repositories → Add*, paste any git URL (a small public repo
   ...
   Repos **already on your machine** work too: `setup.sh` asks where you keep your
   repositories and shares only that folder with the stack (nothing on your disk is visible
   otherwise). If you skipped the question, re-run `./setup.sh` to grant a folder, then
...
requires an explicit, deliberate grant of a single directory — `setup.sh` asks for it
interactively (or take it from `SHIRE_LOCAL_REPOS_DIR`; see
