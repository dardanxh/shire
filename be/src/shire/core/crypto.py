...
when a clone or connection-test needs them. The key comes from `SHIRE_SECRET_KEY`:
...
def _fernet() -> Fernet:
    ...
    logger.warning(
        "SHIRE_SECRET_KEY is not set — encrypting credentials with an INSECURE dev key. "
        "Set SHIRE_SECRET_KEY (e.g. `python -c 'from cryptography.fernet import Fernet; "
