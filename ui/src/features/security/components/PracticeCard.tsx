...
import {
  type DataSafetyPractice,
  useUpdateDataSafetyPracticeMutation,
} from "../api";
...
export function PracticeCard({ practice }: { practice: DataSafetyPractice }) {
  ...
  const { mutate: updatePractice } = useUpdateDataSafetyPracticeMutation(
    practice.id,
  );
  ...
  return (
    <div className="group relative h-full">
      ...
      <button
        type="button"
        onClick={() => updatePractice({ starred: !practice.starred })}
        aria-label={t(
          practice.starred
      ...
      >
        <StarIcon
          className={cn("size-4", practice.starred && "fill-warning")}
        />
      </button>
    </div>
  );
